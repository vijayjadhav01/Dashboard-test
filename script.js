...
function setupMapControls() {
    ...
    mapCommoditySelect.addEventListener('change', function() {
        ...
        // Auto-update KPIs when commodity is selected
        if (this.value) {
...
// NEW: Update National Diet Chart
        ...
        heatmapHTML += `
            <div class="state-row">
                ...
                ${row.costs.map(cost => {
                    ...
                    const blue = Math.floor(255 - (intensity * 100)); // Lighter blue for lower costs
                    const color = `rgb(${blue}, ${blue}, 255)`;
                    
                    return `<div class="day-cell" style="background: ${color};" title="${row.state}: Rs ${Math.round(cost).toLocaleString()}"></div>`;
                }).join('')}
            </div>
        `;
    });
    ...
    const logoPlugin = {
        ...
        beforeDraw: function(chart) {
            ...
            logo.onload = function() {
                const logoWidth = 60;
                const logoHeight = 30;
                const x = chart.width - logoWidth - 15;
                const y = 15;
...
function generateWeeklyChart(weeklyData) {
    ...
    const datasets = selectedCommodities.map((commodity, index) => {
        const data = weeklyData.map((item, idx) => ({
            x: new Date(Date.now() - (weeklyData.length - 1 - idx) * 7 * 24 * 60 * 60 * 1000),
            y: item.prices[commodity]
